...
class Merger(nn.Module):
    def __init__(self, args, vocab):
        super().__init__()
        self.sentence_encoder = Merger.SentenceEncoderRNN(
            args,
            input_vocab=vocab.src.word2id)
        self.bigram_encoder = Merger.BigramCharRNN(
            args,
            output_vocab_size=2,
            input_vocab=vocab.src.char2id)

    def forward(self, bigrams, sents, lengths_bigram, lengths_sent):
        last_hidden_sents = self.sentence_encoder(sents, lengths_sent)
        outputs_bigram = self.bigram_encoder(last_hidden_sents, bigrams, lengths_bigram)
    ...
    class SentenceEncoderRNN(nn.Module):
        def __init__(self,
            ...
            self.lstm = nn.LSTM(args.we_dim,
                                args.rnn_dim,
                                num_layers=args.rnn_layers,
                                bidirectional=True)
            self.fc_h = nn.Linear(args.rnn_dim, args.rnn_dim_char)
            self.fc_c = nn.Linear(args.rnn_dim, args.rnn_dim_char)
            self.relu_h, self.relu_c = nn.ReLU(), nn.ReLU()
            self.dropout = nn.Dropout(0.1)
        ...
        def forward(self, sentences, lengths):
            ...
            h_n = self.relu_h(self.fc_h(hidden[0]))
            c_n = self.relu_c(self.fc_c(hidden[1]))
            return h_n, c_n
...
class Network:
    ...
    class BigramData(Dataset):
        ...
        def __getitem__(self, index):
            ...
            sent = torch.tensor(self.data[index][2], dtype=torch.long)
            return src, label, sent
    ...
    def load_data(self, args, train_split=0.8):
        with open(args.data) as f:
            ...
            src = self.vocab.src.bigrams2charindices([bigram[0].strip() for bigram in data])
            labels = [0 if bigram[1].strip() == '0' else 1 for bigram in data]
            sents = self.vocab.src.words2indices([bigram[2].strip().split() for bigram in data])
            src, labels, sents = src[:args.data_size], labels[:args.data_size], sents[:args.data_size]
            data = list(zip(src, labels, sents))
        ...
        def generate_batch(data_batch):
            src_batch, labels_batch, sent_batch = [], [], []
            lengths_bigram, lengths_sent = [], []
            for src_item, label_item, sent_item in data_batch:
                lengths_bigram.append(len(src_item))
                lengths_sent.append(len(sent_item))
                src_batch.append(src_item)
                labels_batch.append(label_item)
                sent_batch.append(sent_item)

            src_batch = pad_sequence(
                src_batch, padding_value=self.vocab.src.char2id['<pad>'])
            sent_batch = pad_sequence(
                sent_batch, padding_value=self.vocab.src.word2id['<pad>'])
            labels_batch = torch.stack(labels_batch)
            return (src_batch, labels_batch, sent_batch), (lengths_bigram, lengths_sent)
    ...
    def train(self, args):
        ...
        for epoch in range(args.epochs):
            ...
            for iteration, train_batch in enumerate(self.train_iter):
                (bigrams, labels, sents), lengths = train_batch
                self.model.zero_grad()
                bigrams, labels, sents = bigrams.to(self.device), labels.to(self.device), sents.to(self.device)
                tag_scores = self.model(bigrams, sents, *lengths)
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            correct, total = 0, 0
            epoch_loss = 0
            for (bigrams, labels, sents), lengths in self.dev_iter:
                # Loss
                bigrams, labels, sents = bigrams.to(self.device), labels.to(self.device), sents.to(self.device)
                output = self.model(bigrams, sents, *lengths)
    ...
    def predict(self):
        ...
        with torch.no_grad():
            for (bigrams, labels, sents), lengths in self.dev_iter:
                ...
                inputs_sent += list(sents.permute(1, 0).detach().cpu().numpy())

        return (inputs_bigram, inputs_sent), gold, pred
...
def main():
    ...
    parser.add_argument("--vocab", dest='vocab_path',
                        default="/local/ccayral/orthonormalDA/data/coda-corpus/beirut_vocab.json", type=str,
                        help="Path to vocab JSON file.")
    parser.add_argument("--data",
                        default="/local/ccayral/orthonormalDA/data/coda-corpus/beirut_bigrams_dataset.txt", type=str,
                        help="Path to file with bigrams dataset.")
    parser.add_argument("--cpt", default='/local/ccayral/orthonormalDA/model_weights', type=str,
                        help="Directory to save the model checkpoints to.")
    parser.add_argument("--logs", default='/local/ccayral/orthonormalDA/logs', type=str,
                        help="Directory to save the model checkpoints to.")
    ...
    if not args.load:
        ...
        with open(os.path.join(args.logs, args.logdir), 'w') as f:
            for i, result in enumerate(pred):
                ...
                sent = ' '.join(
                    list(map(lambda x: network.vocab.src.id2word[x], inputs[1][i])))
                print(re.sub(r'(<pad>)+?', r'', bigram), file=f, end=' | ')
                print(result, 'gold:', gold[i], file=f, end= ' | ')
                print(re.sub(r'(<pad>)+?', r'', sent), file=f)

...
from alignment_handler.utils import read_corpus, pad_sents, pad_sents_char
...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        """
        char_indices = [[[self.start_of_word] +
                         [self.char2id[c] for c in re.sub(r'<space>', ' ', w)] +
                         [self.end_of_word]
                         for w in s]
                         for s in sents]
        return char_indices
    ...
    def bigrams2charindices(self, bigrams):
        return [[self.char2id[c] for c in b.strip()] for b in bigrams]
